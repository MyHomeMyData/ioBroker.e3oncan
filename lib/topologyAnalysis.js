            ...
            for (const el of elements) {
                ...
                if (!elementMap.has(key)) {
                    elementMap.set(key, {
                        ...
                        busTypeId,
                        devProp,
                        devType: devPropNames[devProp] ?? String(devProp),
...
function buildHtml(json) {
    ...
    const scanInfo = json.scanTime.replace('T', ' ').slice(0, 19);

    return `<div style="font-family:sans-serif;font-size:13px;padding:8px">
<h3 style="margin:0 0 4px 0">E3 CAN Bus Topology</h3>
<p style="margin:0 0 12px 0;color:#666;font-size:11px">Scan: ${scanInfo} UTC &nbsp;|&nbsp; UDS devices: ${json.udsDevices.length} &nbsp;|&nbsp; Topology elements: ${json.topologyElements.length}</p>
<h4 style="margin:0 0 4px 0">UDS-Accessible Devices</h4>
 ...
 */
function buildTopologySummary(topologyData) {
