...
class O3ERawCodec {
    ...
    decode(string_bin) {
        return { val: RawDecode(string_bin), type: 'string' };
    }
    __len__() {
...
class O3EInt {
    ...
    decode(data) {
        return { val: int2val(data.slice(this.offset,this.offset+this.byte_width), 0, this.signed) / this.scale, type: 'number' };
    }
...
class  O3EBool {
    ...
    decode(string_bin) {
        ...
        return { val: (val == 0 ? 'off' : 'on' ), type: 'string' };
    }
...
class O3ESoftVers {
    ...
    decode(string_bin) {
        ...
        return { val: lstv.join('.'), type: 'string' };
    }
    __len__() {
        return this.string_len;
    }
...
class O3ESdate {
    ...
    decode(string_bin) {
        ...
        return { val: dt.toDateString(), type: 'string' };
    }
    __len__() {
        return this.string_len;
    }
...
class O3EStime {
    ...
    decode(string_bin) {
        ...
        return { val: dt.toLocaleTimeString(), type: 'string' };
    }
    __len__() {
        return this.string_len;
    }
...
class O3EDateTime {
    ...
    decode(string_bin) {
        ...
        return {
            val: { DateTime: { val:dt.toLocaleString(), type:'string'}, Timestamp: { val:Math.round(dt.getTime()), type:'number'}},
            type: 'object'
        };
    }
    __len__() {
...
class O3EUtc {
    ...
    decode(string_bin) {
        ...
        return { val: dt.toUTCString(), type: 'string' };
    }
    __len__() {
        return this.string_len;
    }
...
class O3EEnum {
    ...
    decode(string_bin) {
        ...
        return {
            val: { ID: {val: val, type:'number'}, Text: {val: txt, type:'string'} },
            type: 'object'
        };
    }
    __len__() {
    ...
    decode(string_bin) {
        ...
        for (const cdi of Object.values(this.subTypes)) {
            ...
            if (subT.id.toLowerCase() == 'count') {
                count = subT.decode(string_bin.slice(index,index+subT.string_len)).val;
                result[subT.id] = { val: count, type: 'number' };
                index += subT.string_len;
            } else {
                if (('subTypes' in subT)) {
                    // O3EComplexType
                    result[subT.id] = { val: [], type: 'object' };
                    if (count <= 100) {
                        for (let i=0; i<count; i++) {
                            result[subT.id].val.push(subT.decode(string_bin.slice(index,index+subT.string_len)));
                            index += subT.string_len;
                        }
                    } else {
                        result[subT.id].val = [{val: 'Implausible number of elements. Decoding aborted.', type:'string'}];
                    }
                } else {
...
class O3EcosPhi {
    ...
    decode(string_bin) {
        ...
        return { val: val/this.scale, type: 'number' };
    }
