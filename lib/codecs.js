...
function arr2Hex(arr) {
    ...
    var hs = '';
    for (var v in arr) { hs += toHex(arr[v],2) }
    return hs;
}
...
function sint16toVal(j, ofs=0) {
    var v = Math.pow(2,8)*j[ofs+1]+j[ofs];
...
function sint32toVal(j, ofs=0) {
    var v = Math.pow(2,24)*j[ofs+3]+Math.pow(2,16)*j[ofs+2]+Math.pow(2,8)*j[ofs+1]+j[ofs];
...
function RawEncode(string_ascii, len) {
    let string_bin = toByteArray(string_ascii);
...
class O3EInt {
    ...
    encode(string_ascii) {
        if (flag_rawmode == true) {
            ...
            let string_bin = toByteArray(toHex(val, this.byte_width*2));
...
class O3EByteVal {
    ...
    encode(string_ascii) {
        if (flag_rawmode == true) {
            ...
            let val = Math.round(eval(string_ascii));
            let string_bin = toHex(val, this.string_len*2);
            let bytes = toByteArray(string_bin);
...
class  O3EBool {
    ...
    decode(string_bin) {
        ...
        let val = string_bin[this.offset];
        return (val == 0 ? "off" : "on" );
    }
...
class O3EUtf8 {
    ...
    decode(string_bin) {
        ...
        var mystr = '';
        for (var i=this.offset; i<this.offset+this.string_len;i++) { mystr += String.fromCharCode(string_bin[i]); }
...
class O3EUtc {
    ...
    decode(string_bin) {
        ...
        var dt = new Date(uint32toVal(string_bin.slice(0,4),0)*1000);
...
class O3EList {
    ...
    decode(string_bin) {
        ...
        var index = 0;
        var count = 0;
        for (var subType in this.subTypes) {
            var subT = this.subTypes[subType];
