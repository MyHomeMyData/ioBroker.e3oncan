...
throw new Error(`O3EUtf8.encode(): Result too long: ${this.id} - ${String(result.length)} > ${String(this.string_len)}`);
}
...
result = result.concat(val2byteArr(v,2,1,false));
        ...
        if (result.length > this.string_len) {
throw new Error(`O3ESoftVers.encode() result too long: ${this.id} - ${String(result.length)} > ${String(this.string_len)}`);
}
        return result.concat(Array.from(Array(this.string_len-result.length), () => 0));
    }
    decode(string_bin) {
...
class O3EComplexType {
    ...
    encode(data) {
        ...
        if (result.length > this.string_len) {
throw new Error(`O3EComplexType.encode() result too long: ${this.id} - ${String(result.length)} > ${String(this.string_len)}`);
}
        return result.concat(Array.from(Array(this.string_len-result.length), () => 0));
    }
    decode(string_bin) {
