...
class O3EInt {
     ...
     */
    decode(data) {
        const val = int2val(data.slice(0, this.byte_width), this.signed) / this.scale;
        if (this.decimals > 0) {
...
class O3EFloat32 {
    ...
    decode(string_bin) {
        ...
        const val = floatView[0] / this.scale;
        if (this.decimals > 0) {
            const f = 10 ** this.decimals;
            return Math.round(val * f) / f;
        }
...
/**
 *  Codec for UDSonCAN: Discriminated union. Decodes a 1-byte enum value (looked up in
 *  enums[listStr], same as O3EEnum), followed by a payload whose codec is selected from
 *  args.cases by the enum value (falling back to args.default if no case matches).
 */
class O3ESwitch {
    /**
     * @param {number} string_len  Length of raw data
    ...
    constructor(string_len, idStr, _args, _devUnits) {
        ...
        this.id = idStr;
        this.listStr = _args.listStr;
        this.cases = _args.cases;
        this.default = _args.default;
        this.devUnits = _devUnits;
    }
    /**
     ...
     */
    encode(_data) {
        throw new Error('O3ESwitch.encode(): not implemented yet');
    }
    /**
     * @param {Array} string_bin  Raw data (array of bytes)
     */
    decode(string_bin) {
        const val = string_bin[0];
        const txt =
            this.listStr in enums.enums && String(val) in enums.enums[this.listStr]
                ? enums.enums[this.listStr][String(val)]
                : `Enum not found in ${this.listStr}`;
        const result = { ID: val, Text: txt };
        const cdi = this.cases[String(val)] ?? this.default;
        if (!cdi) {
            result.Error = `No matching case for ID ${val}`;
            return result;
        }
        const subT = new O3Ecodecs[cdi.codec](cdi.len, cdi.id, cdi.args, this.devUnits);
        const sub = subT.decode(string_bin.slice(1, 1 + subT.string_len));
