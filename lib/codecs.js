...
class O3EByteVal {
    ...
    encode(data) {
        if (rawmode.getOpMode()) {
            ...
            const result = [val2byteArr(data, 1, 1, false),0,0,0];
            return result.slice(0,this.string_len);
        }
    }
...
class O3EUtf8 {
    ...
    encode(data) {
        ...
        let result = Object.values(encoder.encode(data));
        while (result.length < this.string_len) result = result.concat(0);  // Padding with 0x00
        return result;
...
class O3EDateTime {
    ...
    encode(data) {
        ...
        if (this.timeformat == 'VM') {
            ...
            return([Math.floor(dt.getFullYear()/100),dt.getFullYear()%100,dt.getMonth()+1,dt.getDate(),0x05,
                dt.getHours(),dt.getMinutes(),dt.getSeconds()]);
        } else {
            return(val2byteArr(data.Timestamp,4,0.001,false));
        }
...
class O3EUtc {
    ...
    encode(data) {
        ...
        return val2byteArr(dt.getTime(),4,0.001,false);
