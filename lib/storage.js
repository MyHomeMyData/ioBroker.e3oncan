...
class storageDids {
     ...
     *
     * @param {object} ctx  Caller context
     * @param {any} didStruct  structure of DID
     * @param {any} obj  remaining part to be evaluated
     */
     ...
     * @param {string} key  Dict key
     */
    async storeMetaData(idDict, cdc, key) {
        idDict[key] = {};
        idDict[key]['desc'] = cdc.desc;
        idDict[key]['acc'] = cdc.acc;
        if ('unit' in cdc) {
            idDict[key]['unit'] = cdc.unit;
        }
        if ('acc' in cdc) {
            idDict[key]['acc'] = cdc.acc;
        }
     ...
     * @param {string} key  Dict key
     */
    async setupCodecById(didInfo, key) {
        if (Array.isArray(didInfo)) {
            for (const e of didInfo.values()) {
                this.storeMetaData(this.didsMetaDict, e.args, `${key}${e.id}`);
                if ('subTypes' in e.args) {
                    await this.setupCodecById(e.args.subTypes, `${key}${e.id}.`);
                }
            }
        } else {
            this.storeMetaData(this.didsMetaDict, didInfo.args, `${key}${didInfo.id}`);
            if ('subTypes' in didInfo.args) {
                await this.setupCodecById(didInfo.args.subTypes, `${key}${didInfo.id}.`);
            }
        }
    }
     ...
     *
     * @param {object} ctx  Caller context
     * @param {string} stateBase  base id of affected state
     * @param {string} did  DID
        ...
        if (val != null) {
            ...
            switch (this.opMode) {
                ...
                case this.opModes[2]: // 'udsDidScan'
                    ...
                    await this.storageDids.setupCodecById(didInfo, `${this.config.stateBase}.${didStr}_`); // Add dids meta data to dictionary
