...
class storageDids {
    ...
    async readKnownDids(ctx) {
        ...
        // else return list of common dids for all devices
        if (this.config.device != 'e380') {
            try {
                const baseId = this.config.stateBase+'.info.';
                this.didsDictCommon = await JSON.parse((await ctx.getStateAsync(baseId+this.didsCommonId)).val);
            ...
            if (Object.keys(this.didsDictCommon).length > 0) {
                return(this.mergeDids(this.didsDictCommon,this.didsDictDevSpec));
            } else {
                return(E3DidsDict['common']);
            }
        } else {
            this.didsDictCommon = {};
            this.didsDictDevSpec = {};
...
class storage {
    ...
    async initStates(ctx, opMode) {
        ...
        if (opMode != 'udsDevScan') {
            ...
            this.dids = await this.storageDids.readKnownDids(ctx);
        } else {
            this.dids = {};
            this.dids[ctx.udsDidForScan] = E3DidsDict.common[ctx.udsDidForScan];
        }
    }
    ...
    async mergeToJson(ctx, valSubId, val, obj) {
        // Merge val to obj
        ctx.log.debug('mergeToJson() '+valSubId+' '+JSON.stringify(val)+' '+JSON.stringify(obj));
        obj[valSubId] = val;
        ctx.log.debug('mergeToJson() '+valSubId+' '+JSON.stringify(val)+' '+JSON.stringify(obj));
    }

    /*
    async mergeToJson(ctx, id, valId, val, obj) {
        // Merge val to obj and return result
        let res = obj;
        ctx.log.debug('mergeToJson() '+id+' <=> '+valId+' '+JSON.stringify(val)+' '+JSON.stringify(obj));
        if (id == valId) {
            // Posistion of id found
            await ctx.log.debug('Match. val='+JSON.stringify(val)+'; '+id+' => '+JSON.stringify(res));
            return res;
        } else {
            if (typeof(obj) == 'object') {
                if (Object.keys(obj).length <= 100) {
                    for (const [key, itm] of Object.entries(obj)) {
                        res = await this.mergeToJson(ctx, id+'.'+String(key), valId, val, itm);
                    }
                } else {
                    await ctx.log.error('Did json merge aborted. Too many members ('+String(Object.keys(obj).length)+') '+id);
                    return {};
                }
            } else {
                await ctx.log.debug('Scalar object. val='+JSON.stringify(val)+'; '+id+' => '+JSON.stringify(res));
                return res;
            }
        }
        await ctx.log.debug('NO match. val='+JSON.stringify(val)+'; '+id+' => '+JSON.stringify(res));
        return res;
    }
    */
    ...
    async encodeDataCAN(ctx, ctxWorker, did, data) {
        ...
        if (did in this.dids) {
            ...
            try {
                ...
                await ctx.log.warn('encodeDataCAN(): Could not retreive codec for '+ctxWorker.config.stateBase+'.'+String(did)+'. err='+e.message);
                val = null;
            }
        } else {
    ...
    async decodeDataCAN(ctx, ctxWorker, did, data) {
        ...
        if (did in this.dids) {
            cdi = this.dids[did];     // Infos about did codec
