...
class storageDids {
    ...
    async storeMetaData(idDict, cdc, key, unitsStr) {
        ...
        if ('unit' in cdc) {
            let unit = cdc.unit;
            if (unit == '°C' && unitsStr.includes(this.imperialStr)) {
    ...
    async setupCodecById(didInfo, key, unitsStr) {
        if (Array.isArray(didInfo)) {
            for (const e of didInfo.values()) {
                this.storeMetaData(this.didsMetaDict, e.args, `${key}${e.id}`, unitsStr);
                if ('subTypes' in e.args) {
                    await this.setupCodecById(e.args.subTypes, `${key}${e.id}.`, unitsStr);
                }
            }
        } else {
            this.storeMetaData(this.didsMetaDict, didInfo.args, `${key}${didInfo.id}`, unitsStr);
    ...
    async decodeDid(ctx, ctxWorker, did, cdi, data) {
        ...
        const res = {};
        try {
            codec = await new E3.O3Ecodecs[cdi.codec](cdi.len, cdi.id, cdi.args, ctxWorker.config.devUnits);
        } catch (e) {
...
class storage {
    ...
    async encodeDataCAN(ctx, ctxWorker, did, data) {
        ...
        if (did in this.storageDids.dids) {
            ...
            try {
                const codec = await new E3.O3Ecodecs[cdi.codec](cdi.len, cdi.id, cdi.args, ctxWorker.config.devUnits);
