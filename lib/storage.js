...
class storage {
    ...
    async storeStatistics(ctx, ctxAgent) {
        if (ctxAgent.stat) await ctx.setStateAsync(ctxAgent.config.stateBase+'.info.'+this.config.statName, JSON.stringify(ctxAgent.stat), true);
    }
    ...
    async decodeDataCAN(ctx, ctxAgent, did, data) {
        ...
        if (did in didsDict[this.config.device]) {
            ...
            try {
                ...
                val = raw;
                ctx.log.warn('Codec failed: '+this.config.canIDhex+'.'+String(did));
            }
