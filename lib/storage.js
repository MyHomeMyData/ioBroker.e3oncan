...
class storage {
    ...
    async initStates(ctx, opMode) {
        ...
        if (['standby','normal','udsDidScan'].includes(opMode)) {
            ...
            await ctx.setObjectNotExistsAsync(this.config.stateBase+'.info.'+this.config.didsAllId, {
                type: 'state',
                common: {
            ...
            await ctx.setObjectNotExistsAsync(this.config.stateBase+'.info.'+this.config.didsSpecId, {
                type: 'state',
                common: {
    ...
    async storeKnownDids(ctx, allDids, didsDict) {
        const id = (allDids ? this.config.didsAllId : this.config.didsSpecId);
        await ctx.setStateAsync(this.config.stateBase+'.info.'+id, JSON.stringify(didsDict), true);
    }
