...
class storageDids {
    ...
    async storeObjectJson(ctx, did, idStr, stateId, obj) {
        ...
        try {
            ...
            let type;
            if (obj.type == 'object') {
                val = JSON.stringify(getValues(obj));
                type = 'string';
            } else {
                val = (obj.type == 'number' ? Number(obj.val) : JSON.stringify(obj.val));
                type = obj.type;
            }
            //if (type == 'number') await ctx.delObjectAsync(stateId);
    ...
    async storeObjectTree(ctx, did, idStr, stateId, obj) {
        try {
            if (obj.type == 'object') {
                if (Object.keys(obj).length <= 100) {
                    for (const [key, itm] of Object.entries(obj.val)) {
                        await this.storeObjectTree(ctx, did, idStr, String(stateId)+'.'+String(key).replace(ctx.FORBIDDEN_CHARS,'_').replace('.','_'),itm);
                ...
                await this.storeObjectJson(ctx, did, idStr, stateId, obj);
            }
        }
        catch(e) {
            ctx.log.error('Storage of did '+stateId+'.'+String(did)+' failed. err='+e.message);
