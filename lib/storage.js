    ...
    async setupCodecById(didInfo, key, unitsStr) {
        if (Array.isArray(didInfo)) {
            for (const e of didInfo.values()) {
                ...
                if ('cases' in e.args) {
                    await this.setupSwitchCases(e.args, `${key}${e.id}.`, unitsStr);
                }
            }
        } else {
            ...
            if ('cases' in didInfo.args) {
                await this.setupSwitchCases(didInfo.args, `${key}${didInfo.id}.`, unitsStr);
            }
        }
    }

    /**
     * Register meta data for all branches of an O3ESwitch codec.
     * O3ESwitch.decode() merges complex sub-codec results directly into the parent
     * object (Object.assign), so their fields appear one level up — the case's own
     * id is NOT an intermediate key. Scalar sub-codecs ARE stored under their id.
     *
     * @param {object} switchArgs  args of the O3ESwitch codec (contains cases / default)
     * @param {string} key  key prefix for didsMetaDict (already includes the switch id)
     * @param {string} unitsStr  UnitAndFormats configuration string of device
     */
    async setupSwitchCases(switchArgs, key, unitsStr) {
        const allCases = [...Object.values(switchArgs.cases), ...(switchArgs.default ? [switchArgs.default] : [])];
        for (const caseCdi of allCases) {
            if ('subTypes' in caseCdi.args) {
                // Complex sub-codec: fields are merged into the switch result — no extra id level
                await this.setupCodecById(caseCdi.args.subTypes, key, unitsStr);
            } else {
                // Scalar/enum sub-codec: stored under the case codec's own id
                await this.storeMetaData(this.didsMetaDict, caseCdi.args, `${key}${caseCdi.id}`, unitsStr);
            }
        }
    }
