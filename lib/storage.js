    ...
    async decodeDataCAN(ctx, ctxWorker, did, data) {
                ...
                if (typeof(obj) == 'object') {
                    if (Object.keys(obj).length <= 100) {
                        for (const [key, itm] of Object.entries(obj)) {
                            await storeObjectTree(ctx, String(stateId)+'.'+String(key).replace('.','_'),itm);
                            // No '.' in state id allowed to avoid sub structure
                        }
                    } else {
        ...
        if (did in this.storageDids.dids) {
            ...
            if (cdi.len == data.length) {
                ...
                idStr = cdi.id.replace('.','_');  // No '.' in state allowed to avoid sub structure
                common = true;
                try {
