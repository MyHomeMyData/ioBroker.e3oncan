...
describe('canGatewayChannel.js => gatewayChannel health tracking', () => {
    ...
    it('fires onStopped with a broker-unreachable reason when LWT goes offline', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e', brokerUrl: 'mqtt://test.local' });
        let reason;
        await ch.addListener('onStopped', r => (reason = r));
        client.emit('message', 'open3e/LWT', Buffer.from('online'));
        ...
        expect(reason).to.include('mqtt://test.local');
    });

    it('fires onStopped with a CAN-bus reason when the bus is not running even though mqtt is online', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e' });
        let reason;
        await ch.addListener('onStopped', r => (reason = r));
        client.emit('message', 'open3e/LWT', Buffer.from('online'));
        client.emit('message', 'open3e/status', Buffer.from(JSON.stringify({ can: 'bus-off' })));
        expect(reason).to.include('CAN bus not running');
    });
    ...
    it('keeps the mqtt client alive once unhealthy, unlike an explicit stop()', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e' });
        client.emit('message', 'open3e/status', Buffer.from(JSON.stringify({ can: 'running' })));
        client.emit('message', 'open3e/LWT', Buffer.from('offline'));
        expect(client.ended).to.equal(false);
        // Still watching LWT/status for a possible recovery, so an explicit
        // stop() (e.g. adapter unload) must still work normally.
        ch.stop();
        expect(client.ended).to.equal(true);
    });

    it('fires onRecovered when health returns after having been unhealthy', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e' });
        let recovered = 0;
        await ch.addListener('onRecovered', () => recovered++);
        client.emit('message', 'open3e/LWT', Buffer.from('online'));
        client.emit('message', 'open3e/status', Buffer.from(JSON.stringify({ can: 'running' })));
        client.emit('message', 'open3e/LWT', Buffer.from('offline'));
    ...
    it('does not fire onRecovered without a prior unhealthy transition', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e' });
        let recovered = 0;
        await ch.addListener('onRecovered', () => recovered++);
        client.emit('message', 'open3e/LWT', Buffer.from('online'));
    ...
    it('ignores raw frames while unhealthy but keeps evaluating LWT/status', async () => {
        const { ch, client } = await startedChannel({ baseTopic: 'open3e' });
        let messages = 0;
        let recovered = 0;
        await ch.addListener('onMessage', () => messages++);
        await ch.addListener('onRecovered', () => recovered++);
        client.emit('message', 'open3e/LWT', Buffer.from('online'));
        client.emit('message', 'open3e/status', Buffer.from(JSON.stringify({ can: 'bus-off' })));
        client.emit('message', 'open3e/raw/251', Buffer.from(JSON.stringify({ dlc: 8, data: '0011223344556677' })));
