    ...
    async onUdsStateChange(ctx, ctxWorker, id, state) {
        ...
        if (id.includes('.json.')) {
            ...
            if (valArr) {
                ...
                const dcs = await ctx.idToDCS(id);
                const rawOrg = JSON.parse((await ctx.getStateAsync(dcs.device+'.raw.'+dcs.state.replace('.json.','.raw.'))).val);
                const rawNew = this.storage.arr2Hex(valArr);
                if (JSON.stringify(JSON.parse(state.val)) == JSON.stringify(valJson)) {
                    await ctx.log.debug('JSON ok');
                    if (rawNew == rawOrg) {
                        await ctx.log.debug('Raw ok');
                    } else {
                        await ctx.log.debug('Raw NOK. new <-> org:');
                        await ctx.log.debug(rawNew);
                        await ctx.log.debug(rawOrg);
                    }
                } else {
        ...
        if (id.includes('.tree.')) {
            ...
            if (id.slice(i).includes('.')) {
                ...
                */
                ...
                //const idBase = id.slice(ctx.namespace.length+1,i+id.slice(i).indexOf('.'));
                /*
                await ctx.getDevices(async function (err, obj) {await ctx.log.debug(JSON.stringify(obj));});
                return;
                */
                /*
                await ctx.log.debug(this.config.stateBase);
                await ctx.getChannelsOf(this.config.stateBase, async function (err, obj) {await ctx.log.debug(JSON.stringify(obj));});
                await ctx.getStatesOf(this.config.stateBase,this.config.stateBase+'.tree', async function (err, obj) {await ctx.log.debug(JSON.stringify(obj));});
                return;
                */
                const idBase = id.slice(0,i+id.slice(i).indexOf('.'));
                //await ctx.log.debug(idBase);
                const dcs = ctx.idToDCS(id);
                ctx.log.debug(JSON.stringify(dcs));
                const i_label = ctx.namespace.length+dcs.device.length+dcs.channel.length+dcs.state.length+4;
                await ctx.getStatesOf(this.config.stateBase,this.config.stateBase+'.tree',async function(err,obj) {
                    /*
                    // Probalby better solution based on numbers as marker for arrays. But not working yet.
                    function isNumber(value) { return /^\d+$/.test(value); }
                    function insertDictSubVal(dict, keyArr, isArray, val) {
                        if (keyArr.length == 1) {
                            const key = keyArr[0];
                            if (isArray) dict[key].push(val); else dict[key] = val;
                        } else {
                            const key = keyArr.shift();
                            const nextIsArray = (((keyArr.length) && (isNumber(keyArr[0]))) ? true : false);
                            if (!(key in dict)) if (nextIsArray) dict[key] = []; else dict[key] = {};
                            insertDictSubVal(dict[key],keyArr, nextIsArray, val);
                        }
                    }
                    */
                    ...
                    for (const st of Object.values(obj)) {
                        if (st._id.includes(dcs.state)) {
                            ...
                            const label = st._id.slice(i_label);
                    ...
                    const jsonDict = JSON.parse((await ctx.getStateAsync(idBase.replace('.tree.','.json.'))).val);
                    const rawOrg   = JSON.parse((await ctx.getStateAsync(idBase.replace('.tree.','.raw.'))).val);
                    await ctxWorker.storage.setOpMode('TEST');
                    const rawNew   = ctxWorker.storage.arr2Hex(await ctxWorker.storage.encodeDataCAN(ctx, ctxWorker, did, treeDict));
                    await ctxWorker.storage.setOpMode('normal');
                    //await ctx.log.debug(JSON.stringify(treeDict));
                    if (treeDict) {
                        if (JSON.stringify(treeDict) == JSON.stringify(jsonDict)) {
                            await ctx.log.debug('JSON ok');
                            if (rawNew == rawOrg) {
                                await ctx.log.debug('Raw ok');
                            } else {
                            ...
                            await ctx.log.debug('Size     = '+String(treeDict.size));
                            await ctx.log.debug('treeDict = '+JSON.stringify(treeDict));
                            await ctx.log.debug('.json.   = '+JSON.stringify(jsonDict));
                        }
                    }
                });
