...
class uds {
    ...
    async addSchedule(ctx, schedule, dids) {
        ...
        if (!Object.keys(this.schedules).includes(String(schedule))) {
            ...
            await ctx.log.silly('UDS worker on '+this.config.stateBase+': Added schedule '+String(schedule)+'s '+JSON.stringify(this.schedules[schedule].dids));
        } else {
            // Schedule already exists => add dids
            this.schedules[schedule].dids = this.schedules[schedule].dids.concat(didsArr.map(function(str) { return parseInt(str); }));
            await ctx.log.silly('UDS worker on '+this.config.stateBase+': Added more dids to schedule '+String(schedule)+'s '+JSON.stringify(this.schedules[schedule].dids));
        }
    }
    ...
    async onUdsStateChange(ctx, ctxWorker, id, state) {
        ...
        switch (dcs.channel) {
            case 'json':
                // Change in json data
                byteArr = await this.storage.encodeDataCAN(ctx, this, did, JSON.parse(state.val));
                if (byteArr) {
            ...
            case 'raw':
                // Change in raw data
                byteArr = this.storage.toByteArray(JSON.parse(state.val));
                if (byteArr) {
                    await this.pushCmnd(ctx, 'write', [[did,byteArr]]);
                    setTimeout(function(ctxWorker,did){ctxWorker.cmndsQueue.push({'mode':'read', 'did': did});},2500,this,did);    // Read value after 2500 ms
                } else {
