...
await ctx.clearInterval(this.schedHandle);
    ...
    async addDids(dids) {
        ...
        this.dids = this.dids.concat(didsArr.map(function(str) {
 return parseInt(str); 
}));
    }
...
class uds {
    ...
    constructor(config) {
        ...
        this.storage = new _storage(this.config);
...
await ctxLocal.callback(ctxGlobal, ctxLocal, ['timeout', {did:ctxLocal.data.did,didInfo:{id:'',len:0},val:''}]);
}
            ...
            case 'json':
                ...
                try {
                    ...
                    if (byteArr) {
                        ...
                        ctx.setTimeout(function(ctxWorker,did){
ctxWorker.cmndsQueue.push({mode:'read', did: did});
},2500,this,did);    // Read value after 2500 ms
                    } else {
...
dict[key] = {};
                    ...
                    try {
                        ...
                        if (byteArr) {
                            ...
                            ctx.setTimeout(function(ctxWorker,did){
ctxWorker.cmndsQueue.push({mode:'read', did: did});
},2500,ctxWorker,did);    // Read value after 2500 ms
                        } else {
...
ctx.log.warn(`UDS worker warning on ${this.config.stateBase} (0x${Number(msg.id).toString(16)}): Evaluation of messages overloaded.`);
}
            if ((this.stat.cntTooBusy % 100) == 0) {
ctx.log.debug(`UDS worker warning on ${this.config.stateBase} (0x${Number(msg.id).toString(16)}): Evaluation of messages overloaded. Counter=${String(this.stat.cntTooBusy)}`);
}
            return;
        }
