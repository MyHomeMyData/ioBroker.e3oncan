...
class uds {
    ...
    async cmndsLoop(ctx) {
        ...
        if ( (this.cmndsQueue.length == 0) && (await this.getComState() == 0) ) this.busy = false;
    }

    async onTimeout(ctxGlobal, ctxLocal) {
        const opMode = await ctxLocal.storage.getOpMode();
        if ( (opMode != 'udsDevScan') && ((opMode != 'udsDidScan')) ) {
            ctxGlobal.log.error('UDS timeout on '+String(ctxLocal.data.did)+'@'+ctxLocal.canIDhex);
        }
        ...
        switch (await this.getComState()) {
            ...
            case 1: // waitForFF
                if ( (candata[0] == 0x03) && (candata[1] == 0x7F) && (candata[2] == this.readByDidProt.SIDtx) ) {
                    // Negative response
                    if (this.callback) {
                        this.callback(ctx, this, ['negative response', this.data.did]);
                    } else {
                ...
                if ( (candata.length == 8) && ((candata[0] >> 4) == 0) && (candata[1] == this.readByDidProt.SIDrx) ) {
                    ...
                    if (didRx == this.data.did) {
                        ...
                        if (this.callback) this.callback(ctx, this, ['ok', this.data.did]);
                        await this.setDidDone();
                        break;
                    } else {
                        // Did does not match
                        if (this.callback) {
                            this.callback(ctx, this, ['did mismatch', this.data.did]);
                        } else {
                ...
                if (this.callback) {
                    this.callback(ctx, this, ['bad frame', this.data.did]);
                } else {
            ...
            default:
                if (this.callback) {
                    this.callback(ctx, this, ['bad state value', this.data.did]);
                } else {
