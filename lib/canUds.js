...
class uds {
    constructor(config) {
        ...
        this.config.didsAllId  = 'udsDidsAll';
        this.config.didsSpecId = 'udsDidsSpecific';
    ...
    async initStates(ctx, opMode) {
        ...
        if (['standby','normal','udsDidScan'].includes(opMode)) {
            ...
            await this.storage.storeKnownDids(ctx, true, {});
            await this.storage.storeKnownDids(ctx, false, {});
        }
            ...
            case 1: // waitForFF
                ...
                if ( (candata.length == 8) && ((candata[0] >> 4) == 0) && (candata[1] == this.readByDidProt.SIDrx) ) {
                    ...
                    if (didRx == this.data.did) {
                        ...
                        await this.storage.decodeDataCAN(ctx, this, this.data.did, this.data.databytes.slice(0,this.data.len));
                        await this.setDidDone(0);
                        break;
                    } else {
                        // Did does not match
