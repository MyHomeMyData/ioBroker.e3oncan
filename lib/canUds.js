...
class uds {
    ...
    async stop(ctx) {
        try {
            ...
            // Stop worker:
            this.callback = null;
            if (this.stat.type == 'normal') {
                ctx.unsubscribeStates(ctx.namespace+'.'+this.config.stateBase+'.*');
                ctx.log.info('UDS worker stopped on '+this.config.stateBase);
            } else {
                ctx.log.silly('UDS worker stopped in mode '+this.stat.type+' on '+this.config.stateBase+' ('+this.canIDhex+')');
            }
        } catch (e) {
    ...
    async cmndsLoop(ctx) {
        if ( (await this.storage.getOpMode() != 'standby') && (this.cmndsQueue.length > 0) && (await this.getComState() == 0) ) {
            ...
            switch (cmnd.mode) {
                ...
                case 'write77': {
                    // WriteByDid
                    if ( (cmnd.mode == 'write77') || ((E3services['0x77'].dids) && (cmnd.did[0] in E3services['0x77'].dids)) ) {
                        ...
                        const txAddr = Number(this.config.canID)+E3services['0x77'].offset;
                        const rxAddr = txAddr + 0x10;
                        if (!ctx.E3UdsSID77Workers[rxAddr]) {
                            ctx.E3UdsSID77Workers[rxAddr] = await this.startupUdsWorker(ctx, txAddr);
                        }
    ...
    async onTimeout(ctxGlobal, ctxLocal) {
        ...
        if (['standby','normal'].includes(opMode)) {
            ...
            if (ctxLocal.stat.type == 'service77') {
