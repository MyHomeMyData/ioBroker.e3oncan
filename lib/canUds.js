...
class uds {
    ...
    async startup(ctx, opMode) {
        ...
        if (['standby','normal'].includes(opMode)) await ctx.registerUdsOnStateChange(this, this.userDidsToReadId, this.onUserReadDidsChange);
    }
    ...
    async msgUds(ctx, msg) {
        ...
        if (this.commBusy) {
            ...
            if (this.stat.cntTooBusy == 1) ctx.log.warn('UDS worker warning on '+this.config.stateBase+': Evaluation of messages overloaded on');
            return;
        }
        ...
        switch (await this.getComState()) {
            ...
            case 1: // waitForFF
                if ( (candata[0] == 0x03) && (candata[1] == 0x7F) && (candata[2] == this.readByDidProt.SIDtx) ) {
                    ...
                    if (this.callback) {
                        await this.callback(ctx, this, ['negative response', {'did':this.data.did,'didInfo':{'id':'','len':0},'val':''}]);
                    } else {
                ...
                if ( (candata.length == 8) && ((candata[0] >> 4) == 0) && (candata[1] == this.readByDidProt.SIDrx) ) {
                    ...
                    if (didRx == this.data.did) {
                        ...
                        if (this.callback) {
                            await this.callback(ctx, this, ['did mismatch SF', {'did':this.data.did,'didInfo':{'id':'','len':0},'val':''}]);
                        } else {
                ...
                if ( (candata.length == 8) && ((candata[0] >> 4) == 1) && (candata[2] == this.readByDidProt.SIDrx) ) {
                    ...
                    if (didRx == this.data.did) {
                        ...
                        this.stat.cntCommBadProtocol += 1;
                        if (this.callback) {
                            await this.callback(ctx, this, ['did mismatch MF', {'did':this.data.did,'didInfo':{'id':'','len':0},'val':''}]);
                        } else {
                            ctx.log.error('UDS worker on '+this.config.stateBase+': Did mismatch MF. Expected='+String(this.data.did)+'; Received='+String(didRx));
                        }
                        await this.setDidDone(1000);
                ...
                if (this.callback) {
                    await this.callback(ctx, this, ['bad frame', {'did':this.data.did,'didInfo':{'id':'','len':0},'val':''}]);
                } else {
