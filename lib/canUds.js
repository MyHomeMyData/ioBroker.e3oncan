...
class uds {
    ...
    async startup(ctx, opMode) {
        ...
        if (opMode == 'normal') {
            await ctx.log.debug('UDS worker started on '+this.canIDhex);
        } else {
            await ctx.log.silly('UDS worker started in mode '+opMode+' on '+this.canIDhex);
        }
        await ctx.registerUdsOnStateChange(this, this.userDidsToReadId, this.onUserReadDidsChange);
    }
    ...
    async stop(ctx) {
        ...
        if (opMode == 'normal') {
            await ctx.log.debug('UDS worker stopped on '+this.canIDhex);
        } else {
            await ctx.log.silly('UDS worker stopped in mode '+opMode+' on '+this.canIDhex);
        }
    }
