...
const E3services = require('./didsE3services.json');
...
class uds {
    constructor(config) {
        ...
        this.writeByDidSID77Prot = {
            'idTx'   : this.config.canID+E3services['0x77'].offset,
            'idRx'   : Number(this.config.canID) + 0x10,
            'PCI'    : 0x00,     // Protocol Control Information = length of data +3
    ...
    async startup(ctx, opMode) {
        ...
        if (opMode == 'normal') {
            await ctx.log.info('UDS worker started on '+this.config.stateBase+' with '+String(Object.keys(this.schedules).length)+' active schedule(s).');
        } else {
            await ctx.log.silly('UDS worker started in mode '+opMode+' on '+this.config.stateBase+' with '+String(Object.keys(this.schedules).length)+' active schedule(s).');
        }
    ...
    async startupUdsWorker(ctx, addr) {
        ...
        await udsWorker.initStates(ctx, 'normal');
        await udsWorker.startup(ctx, 'normal');
        return udsWorker;
    }
    ...
    async cmndsLoop(ctx) {
        if ( (await this.storage.getOpMode() != 'standby') && (this.cmndsQueue.length > 0) && (await this.getComState() == 0) ) {
            ...
            switch (cmnd.mode) {
                ...
                case 'write': {
                    // WriteByDid
                    if ( (E3services['0x77'].dids) && (cmnd.did[0] in E3services['0x77'].dids) ) {
                        await ctx.log.debug('UDS worker on '+this.config.stateBase+': cmndLoop()->writeByDid77(): '+String(cmnd.did));
                        // Startup UDS worker for service 77:
                        const service77Worker = await this.startupUdsWorker(ctx, Number(this.config.canID)+2);
                        await service77Worker.writeByDid77(ctx,cmnd.did);
                        // Stop UDS worker for service 77
                        await service77Worker.stop(ctx);
                    } else {
    ...
    async writeByDid77(ctx, didArr) {
        ...
        this.data.D0 = 0x21;
        await this.setDidStart(ctx, did, 'write', len);
        await this.sendFrame(ctx, await this.initialRequestWrite(did, valRaw, len, this.writeByDidSID77Prot));
        await ctx.log.silly('UDS worker on '+this.config.stateBase+': WriteByDid(): '+String(this.canIDhex)+'.'+String(did)+'='+this.storage.storageDids.arr2Hex(valRaw));
    }
