...
class uds {
    ...
    async startup(ctx, opMode) {
        ...
        if (['standby','normal'].includes(opMode)) await ctx.registerUdsOnStateChange(ctx, this, this.userReadByDidId, this.onUserReadDidsChange);
    ...
    async onUserReadDidsChange(ctxGlobal, ctxLocal, state) {
        ...
        if (!state.ack) {
            ...
            await ctxGlobal.log.debug('UDS user command on device '+ctxLocal.config.stateBase+'. Dids='+JSON.stringify(dids));
