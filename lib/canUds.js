...
class uds {
    ...
    async initStates(ctx, createAll) {
        if (createAll) {
    ...
    async getCmnds(ctx) {
        ...
        return(JSON.parse(obj.val));
    }

    async setCmnds(ctx, cmnds, ack) {
        await ctx.setStateAsync(this.cmndQueueId, {val: JSON.stringify(cmnds), ack: ack});
    }
    ...
    async checkDeviceAddress(ctx, did, maxWait, maxTrials) {
        ...
        const opModeOrg = await this.storage.getOpMode();
        await this.storage.setOpMode('udsDevScan');
        for (let i=0; i<maxTrials; i++) {
            const tsAbort = new Date().getTime() + maxWait;
            if (new Date().getTime() < tsAbort) {
                await this.readByDid(ctx, did);
                while ( (new Date().getTime() < tsAbort) && (this.data.state != this.states[0]) ) {
                    await this.sleep(50);
                }
            }
            if (this.storage.udsScanResult) continue;
        }
        await this.storage.setOpMode(opModeOrg);
        //callbackFct(ctx, this.storage.udsScanResult);
    }
    ...
    async cmndLoop(ctx) {
        ...
        while (true) {
            const cmnds = await this.getCmnds(ctx);
            if ( (cmnds.length > 0) && (this.data.state == this.states[0]) ) {
                ...
                await this.setCmnds(ctx, cmnds, true);
            }
            await this.sleep(50);
        }
    }
    ...
    async onTimeoutChange(ctx, obj) {
        if (!obj) {
            if (await this.storage.getOpMode() != 'udsDevScan') {
    ...
    async readByDid(ctx, did) {
        ...
        await ctx.setStateAsync(this.timeoutId,
            { val: false, ack: true, expire: this.config.timeout/1000 });    // Start Timeout monitoring
        this.data.did = did;
