...
class uds {
    ...
    async onUdsStateChange(ctx, ctxWorker, id, state) {
        ...
        if (id.includes(this.storage.storageDids.didsSpecId)) {
            ...
            const oldDidsSpec = JSON.parse(JSON.stringify(this.storage.storageDids.didsDictDevSpec));
            await this.storage.storageDids.readKnownDids(ctx, await this.getWorkerOpMode());
            const unitsStr =
                !this.config.devUnits || this.config.devUnits == 'n/a' ? ctx.udsMasterDevUnits : this.config.devUnits;
            for (const [did, didInfo] of Object.entries(this.storage.storageDids.didsDictDevSpec)) {
                if (did.length > 4) {
                    continue;
                }
                const didStr = this.storage.storageDids.getDidStr(did);
                await this.storage.storageDids.setupCodecById(didInfo, `${this.config.stateBase}.${didStr}_`, unitsStr);
                await ctx.log.info(`continue => ${JSON.stringify(oldDidsSpec[did])}`);
                if (did in oldDidsSpec && JSON.stringify(oldDidsSpec[did]) === JSON.stringify(didInfo)) {
                    continue;
                }
                const idStr = didInfo.id.replace(ctx.FORBIDDEN_CHARS, '_').replace('.', '_');
                const didStateName = `${didStr}_${idStr}`;
                const raw = await this.storage.storageDids.getObjectVal(
                    ctx,
                    `${this.config.stateBase}.raw.${didStateName}`,
                );
                if (raw != null) {
                    const res = await this.storage.storageDids.decodeDid(
                        ctx,
                        this,
                        did,
                        didInfo,
                        this.storage.storageDids.toByteArray(raw),
                    );
                    await ctx.delObjectAsync(`${ctx.namespace}.${this.config.stateBase}.tree.${didStateName}`, {
                        recursive: true,
                    });
                    await this.storage.storageDids.storeObjectTree(
                        ctx,
                        did,
                        res.idStr,
                        `${this.config.stateBase}.tree.${didStr}_${res.idStr}`,
                        res.val,
                    );
                }
            }
    ...
    async msgUds(ctx, msg) {
        ...
        switch (await this.getComState()) {
            ...
            case 3: // waitForFFSFwbd (wait for confirmation)
                ...
                if (candata.length == 8 && candata[0] == this.writeProt.SIDcf && candata[1] == this.writeProt.SIDrx) {
                    ...
                    if (didRx == this.data.did) {
                        ...
                        this.statCommFailed(this, this.data.did);
                        ctx.log.warn(
                            `UDS worker on ${this.config.stateBase}: Did mismatch writeByDid SF. Expected=${String(
                                this.data.did,
                            )}; Received=${String(didRx)}`,
                        );
                        await this.calcStat();
                ...
                if ((await this.getWorkerOpMode()) != 'service77') {
                    ...
                    ctx.log.warn(
                        `UDS worker on ${
                            this.config.stateBase
                        }: Bad frame for writeByDid SF. candata: ${this.storage.storageDids.arr2Hex(candata)}`,
                    );
                    this.stat.cntCommBadProtocol += 1;
                    this.statCommFailed(this, this.data.did);
                    await this.calcStat();
            ...
            case 4: // waitForFFMFwbd
                ...
                if (candata.length == 8 && candata[0] == this.writeProt.FCrx) {
                    // Multi-frame communication confirmed
                    // Send data in slices of 7 bytes
                    let ST = candata[2]; // Separation Time (ms)
                    if (ST < 20 || ST > 127) {
                        ST = 50;
                    } // Accept ST 20 .. 127 ms. Default to 50 ms.
                    this.data.cntFCrx = candata[1] - 1; // Initialize counter till next FCf (-1 => infinite)
                    while (this.data.txPos < this.data.len) {
                        // More data to send
                        await this.sleep(ctx, ST);
                        const frame = [this.data.D0].concat(
                            this.data.databytes.slice(this.data.txPos, this.data.txPos + 7),
                        );
                        ...
                        if (this.data.cntFCrx == 0) {
                            // FCf expected as next frame => interrupt sending of data frames and wait for FCf
                            // Has no effect, if all data was already sent
                            break;
                        } else {
                    ...
                    if (this.data.txPos >= this.data.len) {
                        // All data was sent
                ...
                if ((await this.getWorkerOpMode()) != 'service77') {
                    ...
                    ctx.log.warn(
                        `UDS worker on ${
                            this.config.stateBase
                        }: Bad frame for writeByDid MF. candata: ${this.storage.storageDids.arr2Hex(candata)}`,
                    );
                    this.stat.cntCommBadProtocol += 1;
                    this.statCommFailed(this, this.data.did);
                    await this.calcStat();
            ...
            default:
                ...
                if (this.callback) {
                    ...
                    ctx.log.error(
                        `UDS worker on ${this.config.stateBase}: Bad state value: ${String(await this.getComState())}`,
                    );
                }
                await this.setDidDone(ctx, 2500);
        }
