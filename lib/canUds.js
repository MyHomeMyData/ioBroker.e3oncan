...
class uds {
    ...
    async onUdsStateChange(ctx, ctxWorker, id, state) {
        ...
        let iLabel;     // Index of start of did state id (did_name ...) in full state id (e3oncan ...)
        switch (dcs.channel) {
            case 'json':
                // Change in json data
            ...
            case 'tree':
                // Change in tree data
                iLabel = ctx.namespace.length+dcs.device.length+dcs.channel.length+dcs.state.length+4;
                // Build json object of complete object tree of changed did:
                    ...
                    */
                    ...
                    for (const st of Object.values(obj)) {
                        if (st._id.includes(dcs.state)) {
                            ...
                            const label = st._id.slice(iLabel);
