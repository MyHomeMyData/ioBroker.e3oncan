...
class udsScan {
    ...
    async scanDidsCallback(ctx, ctxWorker, _args) {
        if (ctx.udsScanWorkers.callbackBusy) {
            ctx.log.error('UDS dids scan: '+ctxWorker.config.stateBase+' -  callback busy!');
            return;
        }
        ctx.udsScanWorkers.callbackBusy = true;
        const response = _args[0];
        const result = _args[1];
        if (result) {
            ...
            switch (response) {
                ...
                case 'negative response':
                    if (response == 'ok') {
                        if (result.common) {
                            ctxWorker.storage.didsDictCommon[did] = result.didInfo;
                        } else {
                            // Device specific did. Do not override previous data
                            if (!(did in ctxWorker.storage.didsDictDevSpec)) {
                                ctxWorker.storage.didsDictDevSpec[did] = result.didInfo;
                            }
                        }
        ...
        ctx.udsScanWorkers.callbackBusy = false;
    }
    ...
    async scanUdsDids(ctx, udsAddrs, udsMaxCntDids) {
        ...
        for (const worker of Object.values(this.workers)) {
            await worker.storage.storeKnownDids(ctx);
            await worker.stop(ctx);
        }
        this.workers = {};
