...
class udsScan {
    ...
    async scanUdsDevices(ctx) {
        ...
        // Start energy meter detection in parallel with device scan:
        const detectedMeters = { e380_97: false, e380_98: false, e3100cb: false };
        ...
        const onEnergyMeterMsg = function (msg) {
            if (!energyMeterListening) {
                return;
            }
            if (e380EvenIds.has(msg.id)) {
                detectedMeters.e380_97 = true;
            }
            if (e380OddIds.has(msg.id)) {
                detectedMeters.e380_98 = true;
            }
            if (msg.id === 0x569) {
                detectedMeters.e3100cb = true;
            }
        };
        await ctx.channelExt.addListener('onMessage', onEnergyMeterMsg);
        ...
        const meterList = [
            detectedMeters.e380_97 ? 'E380 at CAN address 97' : null,
            detectedMeters.e380_98 ? 'E380 at CAN address 98' : null,
            detectedMeters.e3100cb ? 'E3100CB' : null,
        ].filter(Boolean);
        await ctx.log.info(
            `UDS device scan: Energy meters detected: ${meterList.length > 0 ? meterList.join(', ') : 'none'}`,
        );
        ctx.detectedEnergyMeters = detectedMeters;
        const e380Name = ctx.config.e380Name || 'e380';
        const e3100cbName = ctx.config.e3100cbName || 'e3100cb';
        for (const [stateId, val] of [
            [`${e380Name}.info.detectedAddr97`, detectedMeters.e380_97],
            [`${e380Name}.info.detectedAddr98`, detectedMeters.e380_98],
            [`${e3100cbName}.info.detected`, detectedMeters.e3100cb],
        ]) {
