...
const uds = require('./canUds').default;
...
await ctx.log.warn(`UDS scan finished. Number of retries / active UDS scans (should be 0): ${String(this.udsScanDidsCntRetries)} / ${String(this.cntUdsScansActive)}`);
}
    ...
    async scanUdsDids(ctx, udsAddrs, udsMaxCntDids) {
        ...
        if (this.cntUdsScansActive < 0) {
await ctx.log.warn(`UDS dids scan finished. Number of active UDS scans (should be 0): ${String(this.cntUdsScansActive)}`);
}
