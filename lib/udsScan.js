...
/**
 * Collect CAN-IDs suggested by the confirmed UDS device table
 * (devices found on the master bus, each with its own device-type-derived
 * `collectCanId`, possibly comma-separated). Shared between the scan's own
 * Collect-device detection listener and main.js's gateway raw-relay setup,
 * so both agree on what counts as a candidate collect ID.
 *
 * @param {Array} [tableUdsDevices]  ctx.config.tableUdsDevices, if any exists yet
 * @returns {Set<number>}  Candidate collect CAN-IDs, deduplicated
 */
function collectIdsFromDevices(tableUdsDevices) {
    const ids = new Set();
    for (const dev of Object.values(tableUdsDevices || {})) {
        if (dev.collectCanId) {
...
/**
 * Scan status indicators used by tab.html to show warnings:
 *
 * | State                                      | anyScanMissing | collectScanDone | scan-warning | rescan-hint |
 * |--------------------------------------------|----------------|-----------------|--------------|-------------|
 * | No scan done                               | true           | false           | shown        | —           |
 * | Old scan (pre-v1.x), no collect data       | false          | false           | —            | shown       |
 * | New v1.x scan, no collect devices found    | false          | true            | —            | —           |
 * | New v1.x scan, collect devices found       | false          | true            | —            | —           |
 *
 * anyScanMissing:  any device has didsScanDone=false (storage.js: Object.keys(didsWritable).length === 0)
 * collectScanDone: info.collect exists in v1.x format (detectedIds array) — set in main.js on adapter start
 */

/**
 * fetch() a URL and parse the response body as JSON, with an explicit return
 * type so callers don't each need their own type annotation for what
 * Response.json() otherwise infers as unknown.
 *
...
class udsScan {
    ...
    /**
     * UDS scanning only ever runs on the external bus, so its transport
     * alone decides which worker class a scan worker needs to be.
     *
     * @param {object} ctx  Caller context
     */
    udsWorkerClass(ctx) {
        return ctx.config.canExtTransport === 'gateway' ? udsGateway.udsGateway : uds.uds;
    }

    /**
     * Delegate device/DID existence discovery to the gateway's own scan
     * engine instead of sweeping candidate addresses/DIDs via individual
     * rawread calls - see docs/raw-gateway-api.md section 5. Sweeping
     * ourselves would fire many requests against the gateway's single
     * bus-owner queue at once, most of which would exceed our own
     * per-request timeout waiting their turn rather than the bus's.
     *
     * Caches the per-ECU found-DID lists on ctx.gatewayScanResult (keyed by
     * hex address, e.g. "0x680") for scanUdsDids() to use afterwards without
     * a second firmware scan.
     *
     * @param {object} ctx  Adapter context
     * @returns {Promise<Array>}  Numeric ECU addresses the gateway found
     */
    async gatewayScanExistence(ctx) {
        const base = ctx.config.canExtGatewayUrl;
        try {
            const startRes = await fetch(`${base}/api/scan`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ mode: 'known' }),
                signal: AbortSignal.timeout(5000),
            });
            if (!startRes.ok) {
                await ctx.log.error(`UDS device scan: Gateway scan could not be started - HTTP ${startRes.status}`);
                return [];
            }

            // "known" mode is normally done in well under a minute per ECU;
            // this is only a ceiling against a stuck/unreachable gateway, not
            // an expected duration.
            const tsAbort = Date.now() + 20 * 60 * 1000;
            let lastLog = 0;
            for (;;) {
                const status = await fetchJson(`${base}/api/status`, { signal: AbortSignal.timeout(5000) });
                const scan = status.scan || {};
                if (scan.phase === 'done') {
                    break;
                }
                if (scan.phase === 'failed') {
                    await ctx.log.error(`UDS device scan: Gateway scan failed. ${scan.message || ''}`);
                    return [];
                }
                if (Date.now() - lastLog >= 5000) {
                    lastLog = Date.now();
                    // The firmware reuses "curDid" for two different things
                    // depending on phase: during "ecus" it is the candidate
                    // ECU CAN address currently being probed (e.g. 1665 ==
                    // 0x681), only during "dids" is it an actual DID number.
                    const curLabel =
                        scan.phase === 'ecus'
                            ? `CAN address 0x${Number(scan.curDid ?? 0).toString(16)}`
                            : `DID ${scan.curDid ?? '?'}`;
                    await ctx.log.info(
                        `UDS device scan: Gateway scan running (${scan.phase}, ${scan.ecus ?? 0} ECU(s) found, ` +
                            `probing ${curLabel}, ${scan.probed ?? 0}/${scan.total ?? 0} probed).`,
                    );
                }
                if (Date.now() > tsAbort) {
                    await ctx.log.error('UDS device scan: Gateway scan timed out waiting for completion.');
                    return [];
                }
                await this.sleep(ctx, 1000);
            }

            const { addrs, didsByAddr } = await this.gatewayFetchScanResult(ctx);
            ctx.gatewayScanResult = didsByAddr;
            return addrs;
        } catch (e) {
            await ctx.log.error(`UDS device scan: Gateway scan failed: ${e.message}`);
            return [];
        }
    }

    /**
     * Fetch the gateway's last completed scan result (GET /api/system) -
     * the firmware persists it on its own storage partition independently of
     * this adapter's process lifecycle, so this is also the fallback
     * scanUdsDids() uses when ctx.gatewayScanResult is missing (e.g. the
     * adapter restarted between the device scan and the DID scan, which
     * saving the confirmed device table in the admin UI normally triggers).
     *
     * @param {object} ctx  Adapter context
     * @returns {Promise<object>}  {addrs: number[], didsByAddr: {[hex]: number[]}}
     */
    async gatewayFetchScanResult(ctx) {
        const base = ctx.config.canExtGatewayUrl;
        try {
            const sys = await fetchJson(`${base}/api/system`, { signal: AbortSignal.timeout(10000) });
            const addrs = [];
            const didsByAddr = {};
            for (const dev of sys.devices || []) {
                // addrHex comes back uppercase ("0x6A1") from the firmware;
                // normalize to lowercase so it matches the hex strings this
                // adapter builds everywhere else (JS's toString(16) is always
                // lowercase) - otherwise every address containing a hex
                // letter would silently look up an empty DID list.
                didsByAddr[dev.addrHex.toLowerCase()] = (dev.dids || []).map(pair => pair[0]);
                addrs.push(dev.addr);
            }
            return { addrs, didsByAddr };
        } catch (e) {
            await ctx.log.error(`Could not fetch the gateway's scan result: ${e.message}`);
            return { addrs: [], didsByAddr: {} };
        }
    }
                    ...
                    if (response == 'ok') {
                        ...
                        if (result.common) {
                            ...
                            if (did in E3DidsVarDict && didLen in E3DidsVarDict[did]) {
                                const existingSpec = ctxWorker.storage.storageDids.didsDictDevSpec[did];
                                if (existingSpec?.protected) {
                                    // User has protected this definition — use it as-is
                                    const reason = existingSpec.reason ? ` Reason: "${existingSpec.reason}"` : '';
                                    ctx.log.debug(
                                        `Variant Did ${ctxWorker.config.stateBase}_${did}: protected by user. Update skipped.${reason}`,
                                    );
                                    ctxWorker.storage.storageDids.didsDictDevCom[did] = existingSpec;
                                    _args[1].didInfo = existingSpec;
                                    if ('acc' in existingSpec.args) {
                                        acc = existingSpec.args.acc;
                                    }
                                } else {
                                    // Definition is available. Use it as device specific definition. Override it, if it already exists.
                                    ...
                                    ctxWorker.storage.storageDids.didsDictDevSpec[did] =
                                        await E3DidsVarDict[did][didLen];
                                    ctxWorker.storage.storageDids.didsDictDevCom[did] =
                                        await E3DidsVarDict[did][didLen]; // Also valid as common did for this device
    ...
    async startupScanUdsDevice(ctx, addr) {
        const WorkerClass = this.udsWorkerClass(ctx);
        const udsWorker = new WorkerClass({
            canID: Number(addr),
            ...
            gatewayBaseUrl: ctx.config.canExtGatewayUrl,
    ...
    async scanUdsDevices(ctx) {
        ...
        const canExtTransport = ctx.config.canExtTransport || 'local';
        const canExtGatewayConfig = {
            brokerUrl: ctx.config.canExtGatewayMqttUrl,
            username: ctx.config.canExtGatewayMqttUser,
            password: ctx.config.canExtGatewayMqttPassword,
        };
        // What currently identifies "the same connection, no need to reconnect" -
        // the interface name for a local bus, the broker URL for a gateway one.
        // Comparing a gateway's channelExtName (a broker URL) against canExtName
        // (a leftover local-interface field) would always look "different" and
        // force a pointless reconnect on every scan.
        const canExtIdentity = canExtTransport === 'gateway' ? canExtGatewayConfig.brokerUrl : canExtName;

        // Startup CAN:
        if (canExtActivated) {
            if (ctx.channelExt && ctx.channelExtName != canExtIdentity) {
            ...
            [ctx.channelExt, ctx.channelExtName] = await ctx.connectToCan(
                ctx.channelExt,
                canExtTransport,
                canExtName,
                canExtGatewayConfig,
        ...
        this.udsScanDidsCntRetries = 0;
        this.cntUdsScansActive = 0;
        if ((ctx.config.canExtTransport || 'local') === 'gateway') {
            const addrs = await this.gatewayScanExistence(ctx);
            await ctx.log.info(`UDS device scan: Gateway scan found ${addrs.length} device(s).`);
    ...
    async startupScanUdsDids(ctx, addr, dids) {
        ...
        const WorkerClass = this.udsWorkerClass(ctx);
        const udsWorker = await new WorkerClass({
            canID: Number(addr),
            stateBase: devName,
            devUnits: devUnits,
            device: 'common',
    ...
    async scanUdsDids(ctx, udsAddrs, udsDidsLimits) {
        ...
        const collectIds = collectIdsFromDevices(ctx.config.tableUdsDevices);
        ...
        const onCollectMsg = function (msg) {
            ...
            if (
                ...
                if (!ctx.detectedCollectCanIds.has(msg.id)) {
                    ctx.log.info(`UDS dids scan: Collect-capable device detected on CAN ID 0x${msg.id.toString(16)}`);
                }
        ...
        this.udsScanDidsCntDone = 0;
        if ((ctx.config.canExtTransport || 'local') === 'gateway') {
            // suppressStateStorage does NOT mean "don't read" - reading and
            // decoding is what detects device-specific variants (by response
            // length) and builds/updates the known-DID metadata
            // (storeKnownDids() below), regardless of whether new state
            // objects get created. That "don't create new objects, only
            // update existing ones" distinction is already handled deeper
            // in storage.js (storeObjectTree()'s own suppressStateStorage
            // check) - transport-agnostic, nothing to duplicate here.
            if (!ctx.gatewayScanResult || Object.keys(ctx.gatewayScanResult).length === 0) {
                // Nothing cached from a device scan in this adapter lifetime
                // (e.g. the adapter restarted since - saving the confirmed
                // device table in the admin UI does that). The gateway kept
                // its result on its own storage partition regardless, so
                // just fetch it again instead of asking for a rescan.
                await ctx.log.info(
                    'UDS dids scan: No cached gateway scan result - fetching the current one from the gateway.',
                );
                const { didsByAddr } = await this.gatewayFetchScanResult(ctx);
                ctx.gatewayScanResult = didsByAddr;
            }
            for (const addr of Object(udsAddrs).values()) {
                const hex = `0x${Number(addr).toString(16)}`;
                let dids = ctx.gatewayScanResult?.[hex] || [];
                if (udsDidsLimits.dids) {
                    const wanted = new Set(udsDidsLimits.dids);
                    dids = dids.filter(d => wanted.has(d));
                }
                await this.startupScanUdsDids(ctx, addr, dids);
                // More generous than the local sweep's 50 ms: each address
                // gets its own worker with its own queue against the same
                // gateway, so starting several close together still adds up,
                // just far less than sweeping candidates would.
