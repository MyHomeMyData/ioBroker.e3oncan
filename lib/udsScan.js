...
class udsScan {
    ...
    constructor() {
        ...
        // UDS device address → Collect CAN ID mapping.
        // Addresses are standardized by Viessmann across all E3 installations.
        // Previously name-based (kept here for reference in case of rollback):
        //   this.udsDevAddress2CanId = { HPMUMASTER: '0x693', EMCUMASTER: '0x451' };
        this.udsDevAddress2CanId = {
            '0x680': '0x693', // HPMUMASTER
            '0x684': '0x691',
            '0x688': '0x695',
            '0x6c3': '0x692',
            '0x6a1': '0x451', // EMCUMASTER
        };
        this.workers = {};
                ...
                if (result) {
                    if (result.did == String(ctx.udsDidForScan)) {
                        ...
                        await mergeDev({
                            ...
                            collectCanId:
                                ctxWorker.canIDhex in ctx.udsScanWorker.udsDevAddress2CanId
                                    ? ctx.udsScanWorker.udsDevAddress2CanId[ctxWorker.canIDhex]
                                    : '',
    ...
    async scanUdsDids(ctx, udsAddrs, udsDidsLimits) {
        ...
        // Detect Collect-capable devices by listening for time messages (21 FA 01 B3 ...):
