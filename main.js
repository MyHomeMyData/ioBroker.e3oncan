...
class E3oncan extends utils.Adapter {
  ...
  async onReady() {
    ...
    if ("e380Name" in this.config) {
      await this.updateDatapointsCommon([
        { devStateName: this.config.e380Name, device: "e380" },
      ]); // E380 Energy Meter
    }
    if ("e3100cbName" in this.config) {
      await this.updateDatapointsCommon([
        { devStateName: this.config.e3100cbName, device: "e3100cb" },
      ]); // E3100CB Energy Meter
    }

    // Setup external CAN bus if required
    // ==================================

    // @ts-expect-error canExtActivated not visible to editor
    ...
    // @ts-expect-error canIntActivated not visible to editor
    if (this.config.canIntActivated) {
      this.cntCanConnDesired++;
      [this.channelInt, this.channelIntName] = await this.connectToCan(
        this.channelInt,
        // @ts-expect-error canIntName not visible to editor
    ...
    if (this.channelExt) {
      await this.setupE3CollectWorkers(
        // @ts-expect-error tableCollectCanExt not visible to editor
        this.config.tableCollectCanExt,
        this.E3CollectExt,
      );
    }
    if (this.channelInt) {
      await this.setupE3CollectWorkers(
        // @ts-expect-error tableCollectCanInt not visible to editor
        this.config.tableCollectCanInt,
        this.E3CollectInt,
      );
    }
    ...
    this.log.debug(
      `Total number of active workers: ${String(this.cntWorkersActive)}`,
    );
  ...
  async updateDatapointsCommon(devices) {
    ...
    for (const dev of Object.values(devices)) {
      ...
      const devDids = new storage.storageDids({
      ...
      if (devDids.didsDevSpecAvail) {
        if (
          ...
          this.log.info(
            `Updating common datapoints to version ${
              didsDictNew.Version
            } for device ${dev.devStateName}`,
          );
          for (const did of Object.keys(devDids.didsDictDevCom)) {
            if (did != "Version" && did in didsDictNew) {
              ...
              const didStateName = `${await devDids.getDidStr(
                did,
              )}_${await devDids.didsDictDevCom[did].id}`;
              const devStruct = await devDids.getDidStruct(
                this,
                [],
                devDids.didsDictDevCom[did],
              );
              const E3Struct = await devDids.getDidStruct(
                this,
                [],
                didsDictNew[did],
              );
              if (JSON.stringify(devStruct) != JSON.stringify(E3Struct)) {
                ...
                if (raw != null) {
                  ...
                  await devDids.storeObjectJson(
                    ...
                    did,
                    res.idStr,
                    `${this.namespace}.${dev.devStateName}.json.${
                      didStateName
                    }`,
                    res.val,
                  );
                  await devDids.storeObjectTree(
                    ...
                    did,
                    res.idStr,
                    `${this.namespace}.${dev.devStateName}.tree.${
                      didStateName
                    }`,
                    res.val,
                  );
                ...
                if (
                  Number(devDids.didsDictDevCom.Version) <
                  Number(this.didsVersionTC)
                ) {
                  ...
                  if (raw != null) {
                    // Update .tree states:
                    this.log.silly(
                      `  > Update type and role of datapoint ${didStateName}`,
                    );
                    const cdi = await didsDictNew[did];
                    const res = await devDids.decodeDid(
                      this,
              ...
              if (did in didsWritable && !(did in devDids.didsWritable)) {
                this.log.silly(
                  `  > Add ${didStateName} to list of writable datapoints`,
                );
  ...
  async updateDatapointsSpecific(devices) {
    ...
    for (const dev of Object.values(devices)) {
      ...
      if (devDids.didsDevSpecAvail) {
        if (
          devDids.didsDictDevCom.Version === undefined ||
          (Number(didsDictNew.Version) >
            Number(devDids.didsDictDevCom.Version) &&
            Number(devDids.didsDictDevCom.Version) < Number(this.didsVersionTC))
        ) {
          this.log.info(
            `Updating device specific datapoints to version ${
              didsDictNew.Version
            } for device ${dev.devStateName}`,
          );
          for (const did of Object.keys(devDids.didsDictDevSpec)) {
            if (did.length <= 4) {
              try {
                ...
                const didStateName = `${await devDids.getDidStr(
                  did,
                )}_${await devDids.didsDictDevSpec[did].id}`;
                ...
                this.log.warn(
                  `  > Could not update did ${
                    did
                  } because of wrong format (expected a number).`,
                );
                continue;
              }
            }
    ...
    if (!channel) {
      try {
        ...
        await this.log.error(
          `Could not connect to CAN-Adapter "${name}" - err=${e.message}`,
        );
        channel = null;
        chName = "";
      }
  ...
  disconnectFromCan(channel, name) {
    if (channel) {
      try {
        ...
        this.log.error(
          `Could not disconnect from CAN "${name}" - err=${e.message}`,
        );
        channel = null;
      }
    }
  ...
  async setupUdsWorkers() {
    ...
    if (
      // @ts-expect-error tableUdsSchedules not visible to editor
      this.config.tableUdsSchedules &&
      // @ts-expect-error tableUdsSchedules not visible to editor
      this.config.tableUdsSchedules.length > 0
    ) {
      // @ts-expect-error tableUdsSchedules not visible to editor
      for (const dev of Object.values(this.config.tableUdsSchedules)) {
        if (dev.udsScheduleActive) {
          ...
          const devRxAddr = devTxAddr + 16;
          await this.E3UdsWorkers[devRxAddr].addSchedule(
            this,
            dev.udsSchedule,
            dev.udsScheduleDids,
          );
          this.log.silly(
            `New Schedule (${String(dev.udsSchedule)}s) UDS device on ${String(
              dev.udsSelectDevAddr,
            )}`,
          );
        }
      }
    ...
    for (const worker of Object.values(this.E3UdsWorkers)) {
      ...
      this.subscribeStates(
        `${this.namespace}.${worker.config.stateBase}.*`,
        this.onStateChange,
      );
  ...
  async onUnload(callback) {
    try {
      ...
      // @ts-expect-error canExtName not visible to editor
    ...
    if (typeof obj === "object" && obj.message) {
        ...
        if (obj.callback) {
          if (!this.udsDevScanIsRunning) {
            ...
            await this.log.debug(
              'Request "getUdsDevice" during running UDS scan!',
            );
            this.sendTo(
              obj.from,
              obj.command,
      ...
      if (obj.command === "startDidScan") {
        if (obj.callback) {
          if (!this.udsDidScanIsRunning) {
            ...
            this.log.silly(`Received data - ${JSON.stringify(obj)}`);
            await this.udsScanWorker.scanUdsDids(
              this,
              this.udsDevAddrs,
              this.udsDidsMaxNmbr,
            );
            ...
            this.log.silly(
              'Request "startDidScan" during running UDS did scan!',
            );
      ...
      if (obj.command === "getUdsDids") {
        if (obj.callback) {
          ...
          if (obj.message && this.udsDevStateNames.includes(obj.message)) {
            const udsDids = new storage.storageDids({
              stateBase: obj.message,
              device: obj.message,
            });
            await udsDids.readKnownDids(this);
            const udsDidsTable = [];
            if (udsDids.didsDevSpecAvail) {
              for (const [did, item] of Object.entries(
                udsDids.didsDictDevCom,
              )) {
                udsDidsTable.push({
                  didId: Number(did),
            ...
            this.sendTo(
              obj.from,
              obj.command,
              { native: { tableUdsDids: udsDidsTable } },
              obj.callback,
            );
          } else {
            this.sendTo(
      ...
      if (obj.command === "getUdsDidsDevSelect") {
        if (obj.callback) {
          ...
          await this.log.silly(
            `Data to send - ${JSON.stringify(selUdsDevices)}`,
          );
          if (selUdsDevices) {
            await this.sendTo(
              obj.from,
